...
export function useVoiceJournal({ userId, dayNumber }: UseVoiceJournalProps) {
  const [isRecording, setIsRecording] = useState(false);
  const [audioURL, setAudioURL] = useState<string | null>(() => {
    return localStorage.getItem(`voice-journal-${userId}-${dayNumber}`);
  });
  ...
  const saveRecording = (base64Audio: string) => {
    try {
      localStorage.setItem(`voice-journal-${userId}-${dayNumber}`, base64Audio);
  ...
  const deleteRecording = () => {
    try {
      localStorage.removeItem(`voice-journal-${userId}-${dayNumber}`);
      setAudioURL(null);
      window.dispatchEvent(new Event('journeyProgressUpdate'));
    } catch (err) {
