...
import { ChevronRight, Home, MessageCircle, Sparkles, Loader2, Brain, Heart } from 'lucide-react';
...
export default function SoulChatPage() {
  ...
  const { journeyHistory, isLoading: isHistoryLoading } = useJourneyHistory();
  ...
  const handleHistorySelect = (entry: any) => {
    ...
    setAnalysis({ triggers: entry.analysis }); // Wrap analysis in triggers object
    setCurrentJourneyId(entry.id);
  };
